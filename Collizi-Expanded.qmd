...
sim_data_Collizi_Expanded <- tibble(
  ...
  ) + hrt_noise - food_stamps * 0.2, to = c(0.1, 1)),
...
```
```{r}
mod <- lm(baseline_Zung ~ prior_mental_health, data = sim_data_Collizi_Expanded)
summary(mod)
```


```{r}
mod_test <- lm(y1_Zung ~ starting_hrt + baseline_Zung * prior_mental_health + food_stamps, data = sim_data_Collizi_Expanded)
summary(mod_test)
```
