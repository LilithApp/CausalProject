...
sim_data_Collizi <- tibble(
  ...
  starting_mental_health = rnorm(n,mean=0,sd=10.5),
  baseline_Zung = (starting_mental_health + 48.4),

  baseline_SCL = starting_mental_health/14.18919 +0.83,
  y1_mental_health = starting_mental_health * 0.82 * starting_hrt,
  y1_Zung_noise = rnorm(n,mean=0,sd=6.5),
  y1_Zung = (baseline_Zung * (y1_mental_health/starting_mental_health)+ y1_Zung_noise),
  y1_SCL_noise = rnorm(n,mean=0,sd=0.18),
  y1_SCL = baseline_SCL * (y1_mental_health/(starting_mental_health*1.33) - y1_SCL_noise)
  )
...
sim_data_Collizi_Expanded <- tibble(
  ...
  baseline_SCL = rescale(pmin(pmax(starting_mental_health/12 + 0.73,0),4),
                         to = c(0,4)),
    state = sample(State_Data$State, n, replace = TRUE, prob = State_Data$Percent.of.Survey),
...
#Zung-Score range 20–44 Normal Range 45–59 Mildly Depressed 60–69 Moderately Depressed 70 and above Severely Depressed

```{r}
ggplot(sim_data_Collizi_Expanded_true, aes(x=y1_Zung)) +
  geom_density()
```
...
```{r}
#total = 35658
count(sim_data_Collizi_Expanded_true)
sim_data_Collizi_Expanded_true %>%
  filter(y1_Zung < 44.0001) %>% 
  count()
#23136 = 0.6488124
sim_data_Collizi_Expanded_true %>%
  filter(y1_Zung > 44 & y1_Zung < 59.0001) %>% 
  count()
#11865 = 0.3327351
sim_data_Collizi_Expanded_true %>%
  filter(y1_Zung > 59 & y1_Zung < 70.0001) %>% 
  count()
#638 = 0.0178917
sim_data_Collizi_Expanded_true %>%
  filter(y1_Zung > 69.99999) %>% 
  count()
#19 = 0.0005328398
```

```{r}
ggplot(sim_data_Collizi_Expanded_false, aes(x=y1_Zung)) +
  geom_density()
```

```{r}
#total = 14342
count(sim_data_Collizi_Expanded_false)
sim_data_Collizi_Expanded_false %>%
  filter(y1_Zung < 44.0001) %>% 
  count()
#4002 = 0.2790211
...
```{r}
ggplot(sim_data_Collizi_Expanded_true, aes(x=y1_SCL)) +
  geom_density()
```
```{r}
ggplot(sim_data_Collizi_Expanded_false, aes(x=y1_SCL)) +
  geom_density()
```
