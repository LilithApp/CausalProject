...
median(chain_zung)
```
#On average people's Zung Score decreased by 8.8 points
...
```{r}
hrt_false_y1_zung <- ggplot(sim_data_Collizi_Expanded_false, aes(x=y1_Zung)) +
  geom_density()
```
...
```
```{r}
grid.arrange(hrt_true_y1_zung, hrt_false_y1_zung, ncol=2)
```
...
```{r}
hrt_true_y1_scl <- ggplot(sim_data_Collizi_Expanded_true, aes(x=y1_SCL)) +
  geom_density()
```
```{r}
hrt_false_y1_scl <- ggplot(sim_data_Collizi_Expanded_false, aes(x=y1_SCL)) +
  geom_density()
```

```{r}
grid.arrange(hrt_true_y1_scl, hrt_false_y1_scl, ncol=2)
```
